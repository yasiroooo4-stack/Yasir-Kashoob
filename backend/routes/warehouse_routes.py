...
async def create_product_batch(
    ...
    """إنشاء دفعة جديدة"""
    batch_number = data.get("batch_number") or f"BTH-{datetime.now().strftime(\"%Y%m%d\")}-{str(uuid.uuid4())[:6].upper()}"
...
async def create_cycle_count(
    ...
    """إنشاء جرد دوري جديد"""
    count_number = f"CC-{datetime.now().strftime(\"%Y%m%d\")}-{str(uuid.uuid4())[:6].upper()}"
...
async def approve_cycle_count(
    ...
    if adjust_stock:
        # تعديل كميات المخزون بناءً على الجرد
        items = await db.cycle_count_items.find(
        ...
        for item in items:
            if item.get("variance") and item["variance"] != 0:
                ...
                movement = {
                    "id": str(uuid.uuid4()),
                    "movement_type": "adjust",
                    "movement_number": f"ADJ-{datetime.now().strftime(\"%Y%m%d\")}-{str(uuid.uuid4())[:6]}",
                    "product_id": item["product_id"],
                    ...
                    "notes": f"تعديل من الجرد الدوري {cycle_count[\"count_number\"]}",
                    "status": "completed",
...
async def create_return(
    ...
    """إنشاء مرتجع جديد"""
    return_number = f"RTN-{datetime.now().strftime(\"%Y%m%d\")}-{str(uuid.uuid4())[:6].upper()}"
...
async def approve_return(
    ...
    movement = {
        ...
        "movement_number": f"RTN-MOV-{str(uuid.uuid4())[:6]}",
        ...
        "notes": f"مرتجع: {product_return[\"return_reason\"]}",
        "status": "completed",
        "movement_date": datetime.now(timezone.utc).isoformat()
    }
    await db.warehouse_movements.insert_one(movement)
...
async def add_product_supplier(
    ...
    # إذا كان المورد الأساسي، إلغاء الأساسي من الموردين الآخرين
...
async def get_inventory_valuation(
    ...
    return {
        ...
        "items": sorted(items, key=lambda x: x["total_value"], reverse=True)[:100]
    }
...
async def get_inventory_turnover(
    ...
    # حساب تكلفة البضاعة المباعة (الصرف)
    ...
    return {
        ...
        "analysis": {
            "rating": "ممتاز" if turnover_rate > 6 else "جيد" if turnover_rate > 4 else "متوسط" if turnover_rate > 2 else "منخفض",
