...
async def send_password_reset_email(email: str, token: str, full_name: str):
    ...
    reset_link = f"{os.environ.get('FRONTEND_URL', 'https://gps-check-in-1.preview.emergentagent.com')}/reset-password?token={token}"
...
async def get_me(current_user: dict = Depends(get_current_user)):
    ...
    # Calculate permissions using the helper function
    user["permissions"] = await calculate_user_permissions(user, employee)
...
async def reset_data_section(section_id: str, current_user: dict = Depends(require_role(["admin"]))):
    ...
    section_collections = {
        ...
        "inventory": ["inventory", "inventory_items", "inventory_solutions"],
...
[API]
url = https://gps-check-in-1.preview.emergentagent.com
username = 
password = 
...
async def bulk_sync_attendance(
    ...
    # تحميل جدول ربط أجهزة ZKTeco بالمراكز مرة واحدة
    ...
    for record in records:
        try:
            ...
            if record.get("fingerprint_id"):
                ...
                # تحويل IP الجهاز إلى اسم المركز باستخدام جدول الربط
                    ...
                    for emp in all_employees_with_fps:
                        for fp in emp.get("additional_fingerprints", []):
                            if str(fp.get("fingerprint_id")) == fp_id:
                                ...
                                if fp_center:
                                    # مطابقة المركز (تقبل أي تطابق جزئي)
                ...
                # خامساً: البحث في البصمات الإضافية بدون شرط المركز (أي بصمة تطابق)
                if not employee:
                    for emp in all_employees_with_fps:
                        for fp in emp.get("additional_fingerprints", []):
                            if str(fp.get("fingerprint_id")) == fp_id:
                                # تطابق بدون تحقق من المركز
                                employee = emp
                                break
                        if employee:
                            break
...
AVAILABLE_PERMISSIONS = [
    ...
    {"id": "employees_edit", "name": "تعديل جدول الموظفين", "name_en": "Edit Employee Table"},
    {"id": "drivers_edit", "name": "تعديل جدول السائقين", "name_en": "Edit Driver Table"},
    {"id": "employee_scheduling", "name": "جدولة الموظفين", "name_en": "Employee Scheduling"},
    {"id": "driver_scheduling", "name": "جدولة السائقين", "name_en": "Driver Scheduling"},
    ...
    {"id": "advance_requests", "name": "طلبات السلف", "name_en": "Advance Requests"},
    {"id": "advance_hr_approve", "name": "موافقة HR على السلف", "name_en": "HR Advance Approval"},
    {"id": "advance_finance_approve", "name": "موافقة المالية على السلف", "name_en": "Finance Advance Approval"},
    ...
    {"id": "warehouse_add_solution", "name": "إضافة محلول", "name_en": "Add Solution"},
    {"id": "warehouse_edit_solution", "name": "تعديل محلول", "name_en": "Edit Solution"},
    {"id": "warehouse_record_consumption", "name": "تسجيل استهلاك", "name_en": "Record Consumption"},
    {"id": "warehouse_reports", "name": "تقارير المخازن", "name_en": "Warehouse Reports"},
    {"id": "warehouse_export", "name": "تصدير بيانات المخازن", "name_en": "Export Warehouse Data"},
    {"id": "warehouse_issue_cleaning", "name": "صرف مواد التنظيف", "name_en": "Issue Cleaning Materials"},
    {"id": "warehouse_issue_ppe", "name": "صرف معدات الوقاية", "name_en": "Issue PPE"},
    {"id": "warehouse_issue_equipment", "name": "صرف المعدات", "name_en": "Issue Equipment"},
    {"id": "warehouse_issue_lab", "name": "صرف مواد المختبر", "name_en": "Issue Lab Materials"},
    {"id": "warehouse_issue_maintenance", "name": "صرف مواد الصيانة", "name_en": "Issue Maintenance"},
    ...
    {"id": "operations", "name": "العمليات", "name_en": "Operations"},
    {"id": "register_task", "name": "تسجيل مهمة", "name_en": "Register Task"},
...
async def calculate_payroll(period_id: str, current_user: dict = Depends(get_current_user)):
    ...
    for emp in employees:
        ...
        while current_date <= end_date_dt:
            date_str = current_date.strftime("%Y-%m-%d")
            day_of_week = current_date.weekday()  # 0=Monday, 4=Friday, 5=Saturday (matches DB format)
        ...
        record = PayrollRecord(
            ...
            day_off=day_off,
            weekly_off_days_count=day_off,
            sick_leave=sick_leave,
            compensation_leave=compensation_leave,
            annual_leave=annual_leave,
            public_holiday=public_holiday,
            official_holidays=public_holiday,
            ...
            leave_days=annual_leave + sick_leave + emergency_leave + exam_leave,
            unpaid_leave=unpaid_leave,
            otp_days=otp_days,
            total_days=working_days + day_off + public_holiday + annual_leave + sick_leave + emergency_leave + absent_days + unpaid_leave,
